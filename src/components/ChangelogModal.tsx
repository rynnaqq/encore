...
export const ChangelogModal: React.FC<ChangelogModalProps> = ({ isOpen, onClose }) => {
  const changelogs = [
    ...
    },
    {
      ...
      color: 'text-[#E195AB]',
      bg: 'bg-[#FFCCE1]/40',
      border: 'border-[#FFCCE1] dark:border-slate-800'
    }
  ];
            ...
            <div className="bg-gradient-to-r from-[#FFF5D7] via-[#FFCCE1] to-[#E195AB] dark:from-slate-800 dark:via-slate-800 dark:to-slate-900 relative p-4 sm:p-6 pb-5 sm:pb-6 shrink-0 border-b border-[#FFCCE1] dark:border-slate-800 transition-colors">
              ...
              <div className="relative z-10 flex items-center gap-4 mt-2">
                <div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 shadow-md flex items-center justify-center shrink-0 border border-[#FFCCE1] dark:border-slate-700">
                  <FileText className="w-6 h-6 text-[#E195AB] dark:text-[#FFCCE1]" />
                </div>
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Changelog</h2>
                  <p className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300">Stay up to date with the latest updates.</p>
                </div>
              </div>
            </div>

            {/* Content Area */}
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 bg-slate-50 dark:bg-slate-900 space-y-6">
              {changelogs.slice(0, 1).map((log, index) => {
                ...
                return (
                  <motion.div
                    ...
                    key={log.version}
                    className="relative pl-6 sm:pl-8 border-l-2 border-[#FFCCE1] dark:border-slate-800 last:border-transparent pb-2 last:pb-0"
                  >
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-[#FFCCE1] dark:hover:border-slate-600 transition-colors">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 gap-2">
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${log.bg} dark:bg-slate-700 ${log.color} border ${log.border} dark:border-slate-600`}>
                            {log.version}
                          </span>
                          <span className="text-sm font-black text-slate-800 dark:text-slate-100">{log.title}</span>
                        </div>
                        <span className="text-[10px] font-mono text-slate-400 dark:text-slate-400 font-bold">{log.date}</span>
                      </div>
            ...
            <div className="p-4 bg-white dark:bg-slate-900 border-t border-[#FFCCE1]/60 dark:border-slate-800/60 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                  onClick={onClose}
                  className="px-6 py-2.5 rounded-xl bg-slate-800 dark:bg-[#E195AB] text-white text-xs font-bold hover:bg-slate-700 dark:hover:bg-[#d68097] transition-all shadow-md cursor-pointer"
                >
                  Awesome, thanks!
                </motion.button>
              </div>
