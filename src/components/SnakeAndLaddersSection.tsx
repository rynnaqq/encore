...
export const SnakeAndLaddersSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('snl_room_updated', (updatedRoom: SNLRoomState) => {
      setRoom(updatedRoom);
      if (updatedRoom.winnerId) {
        setIsRolling(false);
      }
    });

    newSocket.on('snl_dice_rolled', (data: { roll: number, playerId: string }) => {
      handleRemoteRoll(data.roll, data.playerId, newSocket);
    });
  ...
  const handleRemoteRoll = (roll: number, rollingPlayerId: string, currentSocket: Socket) => {
    setIsRolling(true);
    setDiceValue(null);
    ...
    const rollInterval = setInterval(() => {
      ...
      if (rolls >= 10) {
        ...
        setDiceValue(roll);
        setTimeout(() => {
          if (currentSocket && currentSocket.id === rollingPlayerId) {
            processMyTurn(roll, currentSocket); 
          }
          setIsRolling(false);
        }, 500);
      }
  ...
  const processMyTurn = (roll: number, currentSocket: Socket) => {
    setRoom(prev => {
      if (!prev) return prev;
      ...
      if (SNAKES_AND_LADDERS[newPos]) {
        ...
        setTimeout(() => {
          ...
          currentSocket.emit('snl_turn_processed', { newPosition: dest, logMessage: destMsg, isWinner: dest === TOTAL_CELLS });
        }, 800);
      } else {
        currentSocket.emit('snl_turn_processed', { newPosition: newPos, logMessage, isWinner: newPos === TOTAL_CELLS });
      }
      return prev;
    });
  };
      ...
      {!room ? (
        <div className="text-center py-8">
          <button
            ...
            className="w-full py-4 rounded-xl bg-[#E195AB] text-white font-bold hover:bg-[#d88299] transition-colors text-lg"
          >
            Start Hosting Room
          </button>
        </div>
              ...
              {room.players.map((p, idx) => (
                <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-white border-2 border-slate-100">
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-slate-800">{p.name} {p.id === socket?.id && '(You)'}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        ...
        >
          <div className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-[#FFF5D7] border border-[#FFCCE1] text-[#E195AB] font-bold text-sm mb-4">
            <Trophy className="w-4 h-4" /> Multiplayer
          </div>
                ...
                {room.players.map((p, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-bold text-sm text-slate-800">{p.name} {p.id === socket?.id && '(You)'}</div>
                    </div>
